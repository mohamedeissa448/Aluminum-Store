...
export class ManageCollectOrdersComponent implements OnInit {
  ...
  search() {
    ...
    if(this.typeOfSearch =="Order_ShippingWaybill"){
      this.getdataByWaybill()
    }
    else{
      let dataToSend:any = {};
  ...
  getdataByWaybill(){
    this.CollectOrdersService.searchOrdersByWaybill({Waybill:this.searchInput}).subscribe((response:any)=>{
      if(response.message){
        //it means we find one or many orders
        this.isLoading = false;
        this.orders = new MatTableDataSource(response.orders);
