...
import { ShowSupplierFinancialTransactionsLogComponent } from '../show-supplier-financial-transactions-log/show-supplier-financial-transactions-log.component';
...
export class ManageSupplierComponent implements OnInit {
  ...
  data;
  searchKey: string;
  displayedColumns: string[] = ["Code","Name","show financial transactions" , "Category","Contacts","Active", "Actions"];
  ...
  showFinancialTransactions (element){
    ...
    dialogConfig.autoFocus = true;
    dialogConfig.width = "80%";
    dialogConfig.data = { title: "Show Financial Transactions ",id:element._id,Supplier_Code:element.Supplier_Code,Supplier_Name:element.Supplier_Name };

    let dalogRef=this.dialog.open(ShowSupplierFinancialTransactionsLogComponent, dialogConfig);
    dalogRef.afterClosed().subscribe((data)=>{
      this.initializeTable();
    })
  }
