...
import { ShowPaymentLogComponent } from '../show-payment-log/show-payment-log.component';
import { ShowFinancialTransactionsLogComponent } from '../show-financial-transactions-log/show-financial-transactions-log.component';
...
export class ManageAffiliateSellerComponent implements OnInit {
  ...
  displayedColumns: string[] = [
    "Code","Name", "Type","Contacts","show payments log","show financial transactions",
    "Pay","Change Revenue Percentage","Active", "Actions"
  ];
  ...
  showPaymentslog(element){
    const dialogConfig = new MatDialogConfig();
    dialogConfig.autoFocus = true;
    dialogConfig.width = "80%";
    dialogConfig.data = { title: "Show Payments Log",id:element._id,AffiliateSeller_Code:element.AffiliateSeller_Code,AffiliateSeller_Name:element.AffiliateSeller_Name };

    let dalogRef=this.dialog.open(ShowPaymentLogComponent, dialogConfig);
    dalogRef.afterClosed().subscribe((data)=>{
      this.initializeTable();
    })
  }

  showFinancialTransactions (element){
    const dialogConfig = new MatDialogConfig();
    dialogConfig.autoFocus = true;
    dialogConfig.width = "80%";
    dialogConfig.data = { title: "Show Financial Transactions ",id:element._id,AffiliateSeller_Code:element.AffiliateSeller_Code,AffiliateSeller_Name:element.AffiliateSeller_Name };

    let dalogRef=this.dialog.open(ShowFinancialTransactionsLogComponent, dialogConfig);
    dalogRef.afterClosed().subscribe((data)=>{
      this.initializeTable();
    })
  }
