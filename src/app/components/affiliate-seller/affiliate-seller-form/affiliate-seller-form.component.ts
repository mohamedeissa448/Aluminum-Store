...
export class AffiliateSellerFormComponent implements OnInit {
  ...
  this.encryptedString = localStorage.getItem("encryptedString") ? localStorage.getItem("encryptedString") : "";
  ...
  onSubmit() {
    if (this.affiliateSellerService.form.valid) {
      this.affiliateSellerService.encryptUserString(this.affiliateSellerService.form.value)
      .subscribe((response) => {
        if(response){
          this.notificationService.success(":: encrypted Successfully");
          console.log("response",response)
          this.encryptedString = response.encryptedString;
          console.log("encryptedString",this.encryptedString)
          localStorage.setItem("encryptedString",this.encryptedString)
        }
        else
