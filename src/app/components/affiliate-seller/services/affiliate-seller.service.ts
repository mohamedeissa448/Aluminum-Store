...
export class AffiliateSellerService {
  ...
  /****************** */
  encryptUserString(form){
    return this.http
      .post(`${systemSettings.serverURL}/AffiliateSellers/encryptUserString`, {
        textToEncrypt: form.textToEncrypt,
  
      })
      .pipe(
        map((response: any) => {
          if (response.message == true) return response;
          return false;
        })
      );
